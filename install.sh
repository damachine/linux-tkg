...
if [ "$1" = "install" ]; then
  ...
  if [[ "$_distro" =~ ^(Ubuntu|Debian)$ ]]; then
    ...
    ( set -o pipefail; time make ${llvm_opt} ${_make_extra_flags} "${_thread_num[@]}" bindeb-pkg LOCALVERSION=-${_kernel_flavor} 2>&1 | tee "$_where/logs/build.log" )
    ...
    ( set -o pipefail; time make ${llvm_opt} ${_make_extra_flags} "${_thread_num[@]}" binrpm-pkg LOCALVERSION="${_extra_ver_str}" 2>&1 | tee "$_where/logs/build.log" )
    msg2 "Building successfully finished!"
    ...
    ( set -o pipefail; time make ${llvm_opt} ${_make_extra_flags} "${_thread_num[@]}" 2>&1 | tee "$_where/logs/build.log" )
...
if [ "$1" = "uninstall" ]; then
  ...
  for _module_dir in /lib/modules/*tkg*; do
    ...
    if [[ "${_kernel}" = "${_running_kernel}" ]]; then
      if [[ "${_distro}" = "Gentoo" ]]; then
        [[ -f "${_boot_path}/EFI/Linux/gentoo-${_kernel}-old.efi" ]] && _installed_kernels+=("${_kernel}-old")
      else
  ...
  if [[ "${_kernel}" = "${_running_kernel}-old" ]]; then
    if [[ "${_distro}" = "Gentoo" ]]; then
      _path="${_boot_path}/EFI/Linux/gentoo-${_running_kernel}-old.efi"
      [[ -f "${_path}" ]] && _uki_paths+=("${_path}")
    else
      for _path in "${_boot_path}"/EFI/Linux/*"${_running_kernel}"-old.efi; do
        [[ -f "${_path}" ]] && _uki_paths+=("${_path}")
      done
    fi
  elif [[ "${_distro}" = "Gentoo" ]]; then
    _path="${_boot_path}/EFI/Linux/gentoo-${_kernel}.efi"
    [[ -f "${_path}" ]] && _uki_paths+=("${_path}")
  else
    for _path in "${_boot_path}"/EFI/Linux/*"${_kernel}"*.efi; do
      [[ -f "${_path}" ]] && _uki_paths+=("${_path}")
    done
  fi
  ...
  if [[ "${_kernel}" = "${_running_kernel}-old" ]]; then
    for _path in "${_uki_paths[@]}"; do
      plain "  ${_path}"
    done
  else
    for _path in "${_uki_paths[@]}" /lib/modules/"${_kernel}" /usr/src/*"${_kernel}"; do
  ...
  if [[ "${_continue}" =~ ^(Y|y|Yes|yes)$ ]]; then
    if [[ "${_kernel}" = "${_running_kernel}-old" ]]; then
      (( ${#_uki_paths[@]} )) && sudo rm -f -- "${_uki_paths[@]}"
    else
      sudo rm -rf -- \
        "${_uki_paths[@]}" \
        /lib/modules/"${_kernel}" \
