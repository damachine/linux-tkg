...
while IFS= read -r _noextract_extra; do
  [ -n "$_noextract_extra" ] && noextract+=("$_noextract_extra")
done < <(_run_staging_knob _noextract_extra)
unset _noextract_extra
...
build() {
  ...
  (
    ...
    set -o pipefail
    time make ${_force_all_threads} ${llvm_opt} ${_make_extra_flags} LOCALVERSION= bzImage modules 2>&1 | tee "$_where/logs/build.log"
  ) || _build_status=$?
  (( _build_status == 0 )) || return "${_build_status}"
